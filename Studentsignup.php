...
<?php
	...
	if (isset($_POST['submit'])) {
		...
		$result=mysqli_query($connection,$q);
		if (mysqli_num_rows($result)>0) {
			echo "<script>alert('Roll already exists')</script>";
			exit();
		}
		$query="INSERT INTO student (username,fname,mname,lname,year,dept,phone,password,rollno,login)";
		$query .="VALUES('$username','$fname', '$mname','$lname','$year','$dept',$mob,'$p',$roll,'NO')";
