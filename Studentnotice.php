...
<body style="background-image: url('Image/bg4.jpeg'); background-repeat: no-repeat;background-size: cover;-webkit-background-size: cover;
  ...
  -o-background-size: cover;">
	...
	<div class="login">
		<h2 align="center">STUDENT'S NOTICE</h2><hr>
		<form action="Studentnotice.php" enctype="multipart/form-data" method="POST">
		<textarea cols=50 rows=5 required="yes" name ="snotice"></textarea><br>
...
<?php
	...
	$query='';
	$fstore='';
	if(isset($_POST['snotice']))
	$snotice = $_POST['snotice'];
	if(isset($_FILES['notice']))
	{
		$snoticen=$_FILES['notice']['name'];
		$snoticent=$_FILES['notice']['tmp_name'];
		$fstore="Uploads/".$snoticen;
		move_uploaded_file($snoticent, $fstore );
	}
	$snotice = mysqli_real_escape_string($connection, $snotice );
    $snoticen = mysqli_real_escape_string($connection, $snoticen );
   	$snoticent = mysqli_real_escape_string($connection, $snoticent );
	if (isset($_POST['post'])) {
        $query = "INSERT INTO snotice(Snotice_cont,Snotice_file) ";
    	$query .= "VALUES ('$snotice','$snoticen')";
