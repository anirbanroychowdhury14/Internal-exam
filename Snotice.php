	...
	<div class="grid-tile" style="position:relative;left:-5mm;">
			...
			<ul style="line-height: 200%;position: relative;top: -0.3cm;">		
				 <?php 
				 	...
				 	require ('db.php');
					$query = "SELECT * FROM snotice ORDER BY Snotice_id DESC";
    				 $select_all_categories_query = mysqli_query($connection,$query);

    				while($row = mysqli_fetch_assoc($select_all_categories_query)) {
       					$Snotice_cont = $row['Snotice_cont'];
        				$Snotice_file = $row['Snotice_file'];
        				if($Snotice_file=='')
        				echo "<li>{$Snotice_cont}</li>";
        				else
        				echo "<li><a href='Uploads/{$Snotice_file}'>{$Snotice_cont}</a></li>";
    				}
    				?>
    			</ul>
			</div>
